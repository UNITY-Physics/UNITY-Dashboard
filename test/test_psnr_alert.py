...
def _make_history():
    # site_a/T1: 6 nominal-ish points then one flagged latest point
    # site_b/T1: only 2 points -> insufficient history
    ...
    for i, session in enumerate(base_sessions[:-1]):
        rows.append({"Location": "site_a", "Session": session, "Segment": "T1", "PSNR": 30.0 + (i % 2) * 0.1})
    # latest point is a big outlier
    rows.append({"Location": "site_a", "Session": base_sessions[-1], "Segment": "T1", "PSNR": 50.0})

    rows.append({"Location": "site_b", "Session": "2024-01-01 09_00_00", "Segment": "T1", "PSNR": 20.0})
    rows.append({"Location": "site_b", "Session": "2024-01-02 09_00_00", "Segment": "T1", "PSNR": 20.5})
...
def test_evaluate_sites_full_roster():
    ...
    by_key = {(r["site"], r["segment"]): r for r in results}

    assert by_key[("site_a", "T1")]["status"] == FLAGGED
    assert by_key[("site_b", "T1")]["status"] == INSUFFICIENT_HISTORY
    assert by_key[("site_c", "T1")]["status"] == NO_DATA
    assert by_key[("site_c", "T2")]["status"] == NO_DATA

    # every roster site x both segments must be present, even with no data
    assert len(results) == len(roster) * 2
...
def test_evaluate_sites_reports_non_new_latest_value():
    ...
    previous_state = {("site_a", "T1"): {"last_session": "2024-01-07 09_00_00"}}
    results = evaluate_sites(df, ["site_a"], previous_state=previous_state, k=2.5, min_history=5)

    site_a_t1 = next(r for r in results if r["site"] == "site_a" and r["segment"] == "T1")
    assert site_a_t1["is_new"] is False
    assert site_a_t1["status"] == FLAGGED  # still evaluated even though not new
...
def test_evaluate_sites_marks_new_session():
    ...
    site_a_t1 = next(r for r in results if r["site"] == "site_a" and r["segment"] == "T1")
    assert site_a_t1["is_new"] is True


# --- baseline.load_history --------------------------------------------

def test_load_history_requires_segment_column(tmp_path):
    csv_path = tmp_path / "no_segment.csv"
    pd.DataFrame([
        {"Location": "site_a", "Session": "2024-01-01 09_00_00", "PSNR": 30.0},
...
def test_build_html_table_contains_each_site():
    results = [
        {"site": "site_a", "segment": "T1", "status": FLAGGED, "latest_value": 50.0,
         "baseline_mean": 30.0, "baseline_std": 0.1, "baseline_n": 6,
         "latest_session": "2024-01-07 09_00_00", "is_new": True},
        {"site": "site_b", "segment": "T1", "status": INSUFFICIENT_HISTORY, "latest_value": 20.5,
