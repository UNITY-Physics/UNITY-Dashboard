...
# From leave-one-out z-score analysis of src/data/RWE_PSNR.csv (443 points,
# 16 sites): k=2.5 lands at a ~5% historical flag rate. Recompute with
# derive_k.py once enough segmented history has accumulated.
DEFAULT_K = 2.5
MIN_HISTORY = 5
...
def evaluate_sites(df, roster, previous_state=None, k=DEFAULT_K, min_history=MIN_HISTORY):
    """Evaluate every (site, segment) in `roster` against its latest PSNR value.
